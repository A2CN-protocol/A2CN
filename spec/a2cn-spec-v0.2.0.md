...
REQUIRED in a priced session.

An absent `basis` means the basis is unstated. A consumer MUST NOT assume `net`,
`gross`, or any other default for an unstated basis; how it proceeds (for
example, by declining to commit automatically) is its own policy.

`basis` is a label, not an instruction to compute. A consumer MUST NOT convert
amounts between `net` and `gross`; that is a tax calculation and outside this
specification (see Section 9A.9).
...
`deal_type`, `currency`, `basis`, `session_timeout_seconds`, or
`round_timeout_seconds`. These five parameters are fixed at session initiation.
If the initiator considers the responder's reduced `max_rounds` unacceptable, it
MUST send a Withdrawal to close the session.

Because `basis` may be absent, changing it includes adding a `basis` the
SessionInit did not carry. A responder that recognizes `basis` MUST echo it
unchanged. A SessionAck that omits `basis` (for example, from a responder that
predates it) leaves the session's basis unstated, whatever the SessionInit
...
converts between `net` and `gross` (Section 6.3.1).
